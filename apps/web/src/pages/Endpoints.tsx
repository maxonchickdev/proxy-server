    ...
    <div className="space-y-8">
      ...
      <div className="border border-white/20 p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="mb-1 block text-sm text-white/60">Target URL</label>
            <input
              type="url"
              value={targetUrl}
      ...
      <div>
        <h2 className="mb-4 text-lg font-medium">Your endpoints ({endpoints.length})</h2>
        {isLoading ? (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/20">
                  ...
                  <th className="px-4 py-3 text-left text-sm font-medium text-white/60">Slug</th>
                  <th className="px-4 py-3 text-left text-sm font-medium text-white/60">Target URL</th>
              ...
              <tbody className="divide-y divide-white/10">
                {endpoints.map((ep) => (
                  <tr key={ep.id} className="hover:bg-white/5">
                    ...
                    <td className="px-4 py-3 font-mono text-sm text-white/80">{ep.slug}</td>
                    <td className="px-4 py-3 text-sm text-white/60 truncate max-w-[250px]">{ep.targetUrl}</td>
